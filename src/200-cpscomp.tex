...
We use the same notation as Haskell uses for the lambda calculus, \icode{\} and \icode{->} are used for lambda abstraction and a space is used for application. Both programs are valid inputs to LamToWat and contain all five of the lambda calculus constructs. The first program applies a function that adds 1 to its argument \icode{x} to the number 42. The second programs applies a nested function that adds its arguments to the numbers 13 and 29. We choose a program with nested functions and a program without nested functions as examples to display the compilation of this language feature separately. Translated to the \icode{Lam} language the programs are written as follows.
...
Module 
  ...
  (App (INT 0) [INT 41])
\end{lstlisting}
 We can print \icode{Wat} as 'real' WebAssembly that can be compiled and ran by WABT \autocite{wabt}. Our program has become a lot longer and there are many new keywords. WebAssembly requires the programmer to declare some extra information. This information can be deduced from the previous expression, so we can generate it automatically. The first seven lines declare that there is one page of mutable global memory and a mutable heap pointer. There is one function reference of the function \icode{f}. This function has type \icode{_t1}. In order to run the program we need to export the \icode{_start} function. Following the preamble we see the entry point of our program in the form of the function \icode{start} and our original function \icode{f}. Function signatures now include a result type (which is always \icode{i32}) and all locally used variables. When we call a function we indicate the type of that function.
...
CPS conversion will transform the \icode{Lam} data type into the \icode{Cps} data type. By converting our program to continuation-passing style we make control flow and data flow explicit. In practice this means that we generate fresh variable names for intermediate values and have a continuation that indicates the order of expressions. Fresh variables are generated using the state monad. The extra argument to our function is also called a metacontinuation: a continuation in the metalanguage and will be provided by the continuation monad. We use Haskell's \icode{do}-notation to write our monadic conversion function in an imperative style. Our monad \icode{M} will have the following type, the order of transformers is arbitrary.
...
% show normal defintion (with extra argument)
% show greenbook function (chapter 5)
\begin{lstlisting}[language=Haskell]
l2c :: Lam -> M Val
...
(func $_f0 (param $_nv i32) (param $x i32) (param $_k1 i32) (result i32) (local $_x2 i32) (local $__k1 i32)
...
Closure conversion has type \icode{c2c :: Cps -> WriterT [(String,[String],Cps)] (Reader [String]) Cps}. If we ignore the monadic effects for a moment we obtain the simpler type \icode{([(String,[String],Cps)],Cps)}. The \icode{Cps} data type is free to contain \icode{FIX} expressions. We want to guarantee that it does not and function are thus no longer nested. All function definitions should be contained in the function list and their bodies should be made up of expression containing only addition, records, and application.
