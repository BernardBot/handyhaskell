...
Hoisting transforms the \icode{Cexp} data type into the \icode{Cexp} data type with a single \icode{FIX}ed function definition. Hoisting is a non-local transformation: the entire expression tree is affected. \icode{hoist} makes heavy use of 
...
Emitting transforms the \icode{Cexp} data type into the \icode{Wmodule} data type.