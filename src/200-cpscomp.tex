...
% TODO parsec link
LamToWat is split into a front-end and a back-end. The front-end of the compiler is made up of parsing (\icode{parse}), and CPS conversion (\icode{lam2cps}). We will not discuss  parsing in detail, as it is orthogonal to the research in this thesis. We will focus on the transformation that involve the IR. We use \icode{parsec} to build a parser for our compiler. The back-end of LamToWat consist of three transformations on the IR: closure conversion, hoisting, and emitting (\icode{cps2wat}). In the first version of the compiler hoisting and closure conversion are combined in \icode{cps2cps}.
...
% mention WebAssembly output -> terminal ghci
...
% motivate it more -> repeated data types + explosion
% mention lines of code -> show more 
% MEASURE
