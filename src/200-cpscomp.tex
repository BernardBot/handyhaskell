...
App (App (Lam "x" (Lam "y" (Add (Var "x") (Var "y")))) (Int 13)) (Int 29)
\end{lstlisting}
...
Let's see how our example program looks in the \icode{Wat} language.
...
The only real difference is the use of the an \icode{INT} instead of a \icode{LABEL} to indicate a function pointer. We have a pretty-printed version for \icode{Wat} as well which looks as follows. It has a similar structure to \icode{Cps}, but it has some of the keywords of WebAssembly instead of Python.
...
\begin{lstlisting}[language=Haskell]
module
func _f0 (param _nv) (param x) (param _k1)
  _nv = load 1 _nv
  _x2 = x + 1
  _nv = malloc 3
  store 0 _nv x
...
We see that other keywords are use instead of \icode{def}. Moreover, we see \icode{malloc} and \icode{store} combinations instead of records. The first function call is to integer constant \icode{0} instead of \icode{_f0}.
