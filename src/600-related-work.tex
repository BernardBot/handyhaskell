...
\subsection{\label{subsection:cpstypetrans}Typing CPS Transformations}
...
\icode{ExpC} has two type parameter: an environment \icode{ts} and an actual type \icode{t}. If we look at the signature of \icode{Cclosure} we see that it takes two arguments: a function that is well-typed in an empty environment \icode{()} and takes an extra environment argument, and the environment itself. This ensures that the function does not contain any free variables. Note that the closure is agnostic about the type of its environment. This guarantees that two functions that originally had the same type will have the same type when they are closure converted as well. \icode{Copen} opens a closure. It takes a closure argument and an expression that is well-typed in an environment extended by the closure, i.e., the function pointer and the free variables in \icode{env}.
