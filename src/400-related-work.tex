...
\subsection{\label{subsection:proginterface}Programmer Interface}
% blocks
...
% TODO: separation
Freer monads and open unions are better used separately instead of combining them with command trees.

% \subsection{\label{subsection:ctreetype}Command Tree Types}
% % discuss original command tree and problems with types
% % - mapping over subcontinuations: foo (Node cmd ks k) = Node cmd (fmap foo ks) (fmap foo k)
% % - abstract over values (Cmd v)
% % - nested cmds (fun, block)
% % - update environment for generic type in hClosure -> typeclass?
% % - fresh variables vs. asgn/set
