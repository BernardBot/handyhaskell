...
The compiler steps will look like a combination of the first and second version of the compiler with an additional step that translates between the two \icode{Cps} partitions. We are now to indicate what the steps really do, by pattern matching on one specific part and having declarative types.
...
\subsection{Testing}
In order to test if all transformations performed correctly we wrote interpreters for the \icode{Lam}, \icode{Cps}, and \icode{Wat} data types. This allowed us to test programs at different steps of the compilation process. We then compared the results of all interpreters and checked that they were the same. We did not write an interpreter for \icode{Tps}. We simply compared the interpreted result of the compiled \icode{Wat} with that of the other interpreters.

Because we had a parser for the lambda calculus we can write lambda source files and automatically read them. However, because the domain of the lambda calculus is functions and integers and that of WebAssembly and \icode{Wat} is only integers, we need to make sure that our lambda calculus programs have integer results. Otherwise our comparison will fail, which it should do in this case.

To automate the testing process we used Cabal \autocite{haskellcabal}, which allows the user to create a test suite. By simply executing the \icode{cabal test} command all automated tests are run. A folder of lambda calculus source files that are used for testsing is provided in the projects repository.
...
Our compiler is a very simplified version of the compiler described in \citetitle{DBLP:books/daglib/0022396} \autocite{DBLP:books/daglib/0022396}. \citeauthor{DBLP:books/daglib/0022396} discusses a number of other transformations that improve the performance of the generated code.

\begin{itemize}
\item TODO
\end{itemize}
% discuss greenbook
% free variable analysis for lambda calculus
