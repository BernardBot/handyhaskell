...
% optimazitions
It was possible to implement a varation of closure conversion that was optimized for simplicity, nevertheless it is impossible to implement one that optimizes for closure size without breaking the semantic nature of command trees. Command trees do not have access to syntactic variable names and thus can not create minimal closures. Normally, we find the smallest set of variables that are needed to create a closure for a function. We do this by traversing the function body and collecting free variables when we encounter them. This is only possible when we use the same trick we used for hoisting in \icode{hFun}: assume we use a subset of commands that carry their own variable names. Arguably, this is a hack that defeats the semantic nature of command trees instead of utilize it.

% hoisting
We can only hoist after we have reduced the set of commands to syntactic constructors. Since it is the last IR transformation in our compiler this is not a real problem. However, if we would want to do a non-local transformation on the command tree at some other point in the compilation process, we would need to 'syntactify' our command tree entirely. After this non-local transformation we would like to get back to a semantic version of our command tree, where join-points actually bind variables. It is not trivial to go from the syntactic to the semantic command trees. There would need to be a substitution function that captures all syntactic variables.

\subsection{Improved and Partitioned Cps}
% cps similar
...
Our original constructors are here combined with embedded effects. These effects were represented by monad transformers in the first version in the compiler and with the two command \icode{Fresh} and \icode{Block} in the second version. Embedding them inside gives us all the features of a command tree without overhead of projecting and injecting commmands. The continuations of \icode{ADD,RECORD,SELECT} have been changed to be semantic and have type \icode{Val -> Cps a} instead of being syntactic and binding their names directly. We (monadic) bind expressions together by entering their continuation. For example the monadic bind of \icode{ADD} is defined as \icode{ADD v1 v2 k >>= f = ADD v1 v2 (\ x -> k x >>= f)}. We can define the same smart constructors as for the command tree. This time, however, they are simply constructors and not very smart. If we take \icode{ADD} as example again we get the constructor by combining it with \icode{DONE}: \icode{add v1 v2 = ADD v1 v2 DONE}.
...
% The main difference is the use of a signature \icode{sig}. A command tree does not know what set of commands is used. However, it does encode strong type constraints on its subcontinuations and continuation. To enforce these constraints we use the signature. This way a command tells the command tree what comes after itself. More formally a signature has type:
...
% Without commands we can not do anything with our command trees. We used a set of commands that reflect the constructors from the CPS data type. We defined a number of extra commands that will help us compile. Just as in with the command tree used in the second version of the compiler we used constructor like \icode{add} etc. to make working with this tree more pleasant.
