...
Pattern matching is a feature that benefits many functional languages. To compile pattern matching we can use \icode{switch} expressions. \icode{switch} expressions takes a value and a list of expressions. We would need to add a switch expression to \icode{Cps} in the first version LamToWat and a switch command to both \icode{Tree} and \icode{Tps}. Our source language would have \icode{case} statements. These would need to be compiled to \icode{switch} expressions. This translation can be optimized by way of a decision-tree algorithm.
