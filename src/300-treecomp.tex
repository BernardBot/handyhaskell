...
Notice that \icode{Tps} does not use all the things a signature tells it. \icode{Tps} is specialized to mirror the structure and types of \icode{Tps} and thus only uses the \icode{n} and \icode{b} part of the signature.
...
In order to check intermediate results of the compiler we would like to have an interpreter for \icode{Tree} and \icode{Tps}. We can now only print both data types, but we would like to map \icode{Tree} and \icode{Tps} to a certain domain. The modular nature of \icode{Tps} requires some attention to make the interpreter modular as well. We would also like a function that transforms \icode{Tps} back into \icode{Tree} for the right set of commands. This would show the isomorphism between the two and give another check of the compilation process.
