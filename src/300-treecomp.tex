...
\item The argument \icode{p} and return type \icode{r} of the sub-continuations.
...
% blocks
Although command trees provide a useful abstraction for language implementers, it does require knowledge of the block model for control flow. Command trees help the programmer somewhat by providing meta-continuation store, which can be manipulated with the \icode{SetK} and \icode{GetK} commands. The language implementer will still need to wrap certain parts of code inside a block and fetch the right continuation at the right point. The responsibility is now put on the compiler writer, who has to compile the semantic command tree into a syntactic command tree.
